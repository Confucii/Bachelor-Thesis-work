...
class ShuffleNetBlock(torch.nn.Module):
    def __init__(self, in_features, num_groups):
        ...
        self.layers = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=in_features, out_channels=in_features, groups=num_groups, kernel_size=1),
            torch.nn.BatchNorm2d(in_features),
            torch.nn.ReLU(),
            Shuffle(num_groups),
            torch.nn.Conv2d(in_channels=in_features, out_channels=in_features, groups=in_features, kernel_size=3, padding=1, stride=1), # DW convolution?
            torch.nn.BatchNorm2d(in_features),
            torch.nn.Conv2d(in_channels=in_features, out_channels=in_features, groups=num_groups, kernel_size=1),
            torch.nn.BatchNorm2d(in_features)
        )
...
class ShuffleNet(torch.nn.Module):
    def __init__(self):
        ...
        self.layers = torch.nn.Sequential(
            ...
            torch.nn.Linear(in_features=num_channels, out_features=5749),
