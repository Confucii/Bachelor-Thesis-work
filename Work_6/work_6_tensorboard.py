...
class Model(torch.nn.Module):
    def __init__(self, args):
        ...
        conv1_pretrained = self.resnet_pretrained.conv1 # (out_planes, 3, 7, 7)
        self.resnet_pretrained.conv1 = torch.nn.Conv2d(
            in_channels=1,
            out_channels=conv1_pretrained.out_channels,
            kernel_size=conv1_pretrained.kernel_size,
            padding=conv1_pretrained.padding,
            stride=conv1_pretrained.stride,
            bias=conv1_pretrained.bias
        )
        self.resnet_pretrained.conv1.weight.data = torch.mean(conv1_pretrained.weight.data, dim=1, keepdim=True) # (out_planes, 1, 7, 7)
    ...
    def forward(self, x):
        return self.resnet_pretrained.forward(x)
...
for epoch in range(1, args.epochs + 1):
    ...
    for dataloader in [data_loader_train, data_loader_test]:
        ...
        logits_with_labels_and_img = []

        for x, y in tqdm(dataloader, desc=mode):
            x = x.to(args.device)
            y = y.to(args.device).squeeze().long()
            ...
            acc = np.mean((np_y_prim_idx == np_y_idx) * 1)
            metrics[f'{mode}_acc'].append(acc)

            for idx in range(len(np_y_idx)):
                conf_matrix[np_y_prim_idx[idx], np_y_idx[idx]] += 1
            if len(logits_with_labels_and_img) < 1000:
                logits_with_labels_and_img.append((
                    np_y_prim[idx],
        ...
        embs, labels, imgs = list(zip(*logits_with_labels_and_img))
