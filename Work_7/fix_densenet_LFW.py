...
class DatasetLFW(torch.utils.data.Dataset):
    def __init__(self):
        ...
        self.X, self.Y = sklearn.datasets.fetch_lfw_people(return_X_y=True, slice_=(slice(62, 187), slice(62, 187)))
...
class DenseBlock(torch.nn.Module):
    def __init__(self, in_features, num_chains=4):
        ...
        for i in range(num_chains):
            ...
            self.chains.append(torch.nn.Sequential(
                ...
                torch.nn.Conv2d(in_channels=out_features, out_channels=in_features, kernel_size=3, padding=1, stride=1)
            ))

    def parameters(self):
        # list(self.chains[0].parameters()) + list(self.chains[1].parameters()) ...
    ...
    def forward(self, x):
        ...
        for chain in self.chains:
            ...
            list_out.append(out)
            inp = torch.cat(list_out, dim=1) # (B, C, W, H)
        return inp
...
class TransitionLayer(torch.nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.layers = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=in_features, out_channels=out_features, kernel_size=1, stride=1, padding=0),
...
class DenseNet(torch.nn.Module):
    def __init__(self):
        ...
        self.layers = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=1, out_channels=self.num_channels, kernel_size=7, stride=2, padding=1),
            DenseBlock(in_features=self.num_channels),
            TransitionLayer(in_features=self.num_channels + 4 * self.num_channels, out_features=self.num_channels),
            DenseBlock(in_features=self.num_channels),
            TransitionLayer(in_features=self.num_channels + 4 * self.num_channels, out_features=self.num_channels),
            DenseBlock(in_features=self.num_channels),
            TransitionLayer(in_features=self.num_channels + 4 * self.num_channels, out_features=self.num_channels),
            ...
            Reshape(target_shape=(-1, self.num_channels)),
            torch.nn.Linear(in_features=self.num_channels, out_features=5749),
...
for epoch in range(1, 1001):
    ...
    for data_loader in [data_loader_train, data_loader_test]:
        ...
        for x, y in data_loader:
            ...
            loss = torch.sum(-y*torch.log(y_prim[range(len(y)), y] + 1e-8))
